...
if (!empty($_POST)) {
	...
	$id_d= checkInput($_POST['nom_docteur']);
	$isSucces = true;
	...
	if ($isSucces && $isUploadSucces) {
		$db = Database::db();
		$statement = $db->prepare("INSERT INTO `patients` ( `pat_prenom`, `pat_nom`, `pat_age`, `pat_adresse`, `pat_phone`, `pat_image`, `pat_email`, `pat_symptone`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
		$statement->execute(array($firstname,$name,$age,$adresse,$phone,$image,$email,$symptone));
           		 $statements = $db->prepare('SELECT * FROM `patients` WHERE `pat_prenom`="'.$firstname.'"');
		      $statements -> execute(array($id));
    $items = $statements->fetch();
    $id=$items['id'];
		$statemente = $db->prepare("INSERT INTO `traitement` (`id_docteur`, `id_patient`) VALUES ('?', '?')");
		$statemente->execute(array($id_d,$id));
		Database::discon();
		...
		<div class="col-md-8" style="background: url('images/9.png');">
			...
			<div class="form-group col-md-8">
	            ...
	            <select  id="category" name="nom_docteur">
	            	<?php
	            	$db = Database::db();
	            	foreach ($db->query('SELECT * FROM docteur INNER JOIN bloc ON bloc.id_docteur=docteur.id INNER JOIN secteur ON secteur.id_secteur=bloc.id_secteur WHERE secteur.id_secteur="'.$id_st.'"') as $row) {
